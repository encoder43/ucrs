...
from utils.general import check_requirements, check_file, check_dataset, xyxy2xywh, xywh2xyxy, xywhn2xyxy, xyn2xy, \
    segment2box, segments2boxes, resample_segments, clean_str, target2mask
...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
    ...
    # Optimize dataloader for speed and memory: persistent_workers reduces overhead, prefetch_factor improves pipeline
    dataloader = loader(dataset,
                        batch_size=batch_size,
                        num_workers=nw,
                        sampler=sampler,
                        pin_memory=True,
                        persistent_workers=nw > 0,  # Keep workers alive between epochs to reduce overhead
                        prefetch_factor=2 if nw > 0 else None,  # Prefetch batches for better pipeline
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if msk is not None:
            ...
            # Fallback: create zero mask and unit weight if mask is None
            h, w = img.shape[1], img.shape[2]
            mask = np.zeros((1, h // 8, w // 8), dtype=np.float32)
            weight = np.ones((1, h // 8, w // 8), dtype=np.float32)
    ...
    def collate_fn(batch):
        ...
        for i, l in enumerate(label):
            if len(l) > 0:  # Only assign batch index if labels exist
                l[:, 0] = i  # add target image index for build_targets()
        # Handle empty label tensors correctly - torch.cat works fine with empty tensors
        label_cat = torch.cat(label, 0)  # This works even if some tensors are empty
        return torch.stack(img), label_cat, torch.stack(mask), torch.stack(weight), path, shapes
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        # Handle both Windows and Unix paths for mask location
        # Replace 'images' directory with 'masks' directory
        path_norm = path.replace(os.sep, '/')
        if '/images/' in path_norm:
            mask_path = path_norm.replace('/images/', '/masks/')
        elif '\\images\\' in path:
            mask_path = path.replace('\\images\\', '\\masks\\')
        else:
            # Fallback: try replacing 'images' with 'masks' in path parts
            path_parts = path_norm.split('/')
            if 'images' in path_parts:
                idx = path_parts.index('images')
                path_parts[idx] = 'masks'
                mask_path = '/'.join(path_parts)
            else:
                mask_path = path.replace('images' + os.sep, 'masks' + os.sep)
        mask_path = mask_path.replace('.jpg', '.npy').replace('.jpeg', '.npy')
        # Convert back to native path separator if needed
        if os.sep != '/':
            mask_path = mask_path.replace('/', os.sep)
        # mask = np.load(mask_path).astype(np.float32) if os.path.exists(mask_path) else None
        # shape(h,w,2)
        ...
        if r != 1:  # if sizes are not equal
            ...
            img = cv2.resize(img, (int(w0 * r), int(h0 * r)), interpolation=interpolation)
            if mask is not None:
                assert tuple(mask.shape[:2]) == (h0, w0)
                mask = cv2.resize(mask, (int(w0 * r), int(h0 * r)), interpolation=interpolation)
            
        return (img, mask), (h0, w0), img.shape[:2]  # img, hw_original, hw_resized
    else:
        raise NotImplementedError
...
def norm_imgs(imgs: torch.Tensor, model):
    device, dtype = imgs.device, imgs.dtype
    imgs /= 255.
    global _mean, _std
    if _mean.device != device or _mean.dtype != imgs.dtype:
        _mean = _mean.to(device).to(dtype)
        _std = _std.to(device).to(dtype)
    
    cfg_path = getattr(model, 'yaml_file', 'yolov5m.yaml')
    if 'yolo' in cfg_path:
        pass
    elif 'retina' in cfg_path or 'gpvit' in cfg_path:
        imgs = (imgs - _mean) / _std
    elif 'rtmdet' in cfg_path:
        imgs = (imgs - _mean) / _std
        imgs = imgs.flip(1)
    else:
        raise NotImplementedError(f'Please manually configure image normalization for {cfg_path}')
    
    return imgs.to(dtype)
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        # Load image
        (img, msk), _, (h, w) = load_image(self, index)
        assert msk is not None

        # place img in img4
        if i == 0:  # top left
            img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
            msk4 = np.full((s * 2, s * 2, msk.shape[2]), 0.0, dtype=msk.dtype)  # base mask with 4 tiles
            msk4[..., 1] = 1.0  # base weight with 4 tiles
...
def letterbox(img, new_shape=(640, 640), msk=None, color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32):
    ...
    if shape[::-1] != new_unpad:  # resize
        ...
        msk = cv2.resize(msk, new_unpad, interpolation=cv2.INTER_LINEAR) if msk is not None else None
    top, bottom = int(round(dh - 0.1)), int(round(dh + 0.1))
    left, right = int(round(dw - 0.1)), int(round(dw + 0.1))
    img = cv2.copyMakeBorder(img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=color)  # add border
    if msk is not None:
        msk = cv2.copyMakeBorder(msk, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(0., 1.))  # add border
        return img, msk, ratio, (dw, dh)
    else:
        return img, ratio, (dw, dh)


def random_perspective(img, msk=None, targets=(), segments=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0,
                       border=(0, 0)):
    ...
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        if perspective:
            ...
            if msk is not None:
                msk = cv2.warpPerspective(msk, M, dsize=(width, height), borderValue=(0., 1.))
        else:  # affine
            img = cv2.warpAffine(img, M[:2], dsize=(width, height), borderValue=(114, 114, 114))
            if msk is not None:
                msk = cv2.warpAffine(msk, M[:2], dsize=(width, height), borderValue=(0., 1.))
