...
class HeatMapParser(nn.Module):
    def __init__(self, c, ratio=8, threshold=0.5, mask_only=False, cluster_only=False, use_uncertainty=False):
        super().__init__()
        self.c = c
        self.ratio = ratio
        ...
        # Optionally initialize UncertaintySlicer
        if use_uncertainty:
            try:
                from models.uncertainty_slicer import UncertaintySlicer
                self.uncertainty_slicer = UncertaintySlicer(
                    ratio=ratio,
                    threshold=threshold,
                    uncertainty_weight=1.0
                )
            except ImportError:
                warnings.warn('UncertaintySlicer not available. Falling back to standard slicing.')
                self.use_uncertainty = False
                self.uncertainty_slicer = None
        else:
            self.uncertainty_slicer = None
    ...
    def forward(self, x):
        ...
        # Check if uncertainty masks are available (from UncertaintySegmenter)
        uncertainty_mask = None
        use_uncertainty = getattr(self, 'use_uncertainty', False)
        if use_uncertainty and getattr(self, 'uncertainty_slicer', None) is not None:
            # Try to get uncertainty mask from parent model
            # The mask is stored on the Model instance during forward pass in yolo.py
            try:
                # Access via parent module (HeatMapParser is a child of Model)
                for module in self.modules():
                    if hasattr(module, 'pred_mask_uncertainty') and module.pred_mask_uncertainty is not None:
                        uncertainty_mask = module.pred_mask_uncertainty
                        # Extract single channel if multi-channel
                        if uncertainty_mask.dim() == 4 and uncertainty_mask.shape[1] > 1:
                            uncertainty_mask = uncertainty_mask[:, 0, :, :]
                        uncertainty_mask = uncertainty_mask.detach()
                        break
            except:
                # Fall back to standard slicing if uncertainty access fails
                pass
        ...
        if self.training:
            ...
            # Training mode: use uni_slicer (no uncertainty slicing in training for now)
            return self.uni_slicer(x, mask_pred, self.ratio, self.threshold * 1. + 0., device=device)
        else:
            # Inference mode: use uncertainty slicing if available
            if use_uncertainty and getattr(self, 'uncertainty_slicer', None) is not None and uncertainty_mask is not None:
                # Probabilistic slicing with uncertainty
                total_clusters = self.uncertainty_slicer.probabilistic_slice_fast(
                    mean_mask=mask_pred,
                ...
                # Standard slicing (backward compatible)
                total_clusters = self.ada_slicer_fast(mask_pred, self.ratio, self.threshold * 1.0 + 0.)
            
            # Debug: Check heatmap values and cluster count
            if not self.training:
                mask_max = mask_pred.max().item()
                mask_min = mask_pred.min().item()
                mask_mean = mask_pred.mean().item()
                total_cluster_count = sum(len(clusters) for clusters in total_clusters)
                if total_cluster_count == 0:
                    # print(f"\nDEBUG HeatMapParser: No clusters found!")
                    # print(f"  mask_pred stats: min={mask_min:.4f}, max={mask_max:.4f}, mean={mask_mean:.4f}")
                    # print(f"  threshold={self.threshold:.4f}, ratio={self.ratio}")
                    # print(f"  activated pixels: {(mask_pred >= self.threshold).sum().item()}")
                    # Try with lower threshold as fallback
                    # Use adaptive threshold based on actual mask values
                    try:
                        if mask_max > 0.0001:  # If there are any non-zero values
                            # Strategy: Use percentile-based threshold for normal cases,
                            # but use very aggressive threshold for very low values
                            if mask_max < 0.01:
                                # Very low values: use 50% of max as threshold
                                lower_thresh = mask_max * 0.5
                                # print(f"  Very low mask values detected, using 50% of max: {lower_thresh:.6f}")
                            else:
                                # Use percentile-based threshold: take top 10% of values as threshold
                                mask_flat = mask_pred.flatten()
                                sorted_values = torch.sort(mask_flat, descending=True)[0]
                                percentile_idx = max(1, int(len(sorted_values) * 0.1))  # Top 10%
                                lower_thresh = sorted_values[percentile_idx].item()
                                # Ensure threshold is at least 0.1 * max to avoid being too low
                                lower_thresh = max(lower_thresh, mask_max * 0.1)
                                lower_thresh = min(lower_thresh, 0.3)  # Cap at 0.3
                                # print(f"  Trying adaptive threshold: {lower_thresh:.4f} (based on top 10% percentile)")
                            
                            total_clusters = self.ada_slicer_fast(mask_pred, self.ratio, lower_thresh)
                            total_cluster_count = sum(len(clusters) for clusters in total_clusters)
                            # print(f"  Clusters found with adaptive threshold: {total_cluster_count}")
                            
                            # If still no clusters, try even lower threshold
                            if total_cluster_count == 0 and mask_max > 0.0001:
                                # Last resort: use 30% of max
                                last_resort_thresh = mask_max * 0.3
                                # print(f"  Last resort: trying very low threshold: {last_resort_thresh:.6f} (30% of max)")
                                total_clusters = self.ada_slicer_fast(mask_pred, self.ratio, last_resort_thresh)
                                total_cluster_count = sum(len(clusters) for clusters in total_clusters)
                                # print(f"  Clusters found with last resort threshold: {total_cluster_count}")
                        else:
                            # print(f"  WARNING: mask_max ({mask_max:.6f}) is too low, cannot generate clusters")
                            # Fallback: Generate default grid clusters when mask is completely zero
                            # This allows the model to still process images even without mask files
                            if mask_max == 0.0:
                                # print(f"  Generating default grid clusters (mask file likely missing)")
                                # Generate a grid of clusters covering the entire image
                                cluster_w = make_divisible(nx / self.ratio, 4)
                                cluster_h = make_divisible(ny / self.ratio, 4)
                                default_clusters = []
                                for bi in range(bs):
                                    bi_clusters = []
                                    for yy in range(0, ny, cluster_h):
                                        for xx in range(0, nx, cluster_w):
                                            x1, y1 = xx, yy
                                            x2, y2 = min(xx + cluster_w, nx), min(yy + cluster_h, ny)
                                            if x2 > x1 and y2 > y1:  # Valid cluster
                                                bi_clusters.append([x1, y1, x2, y2])
                                    default_clusters.append(torch.tensor(bi_clusters, device=device, dtype=torch.long) if len(bi_clusters) > 0 else torch.zeros((0, 4), device=device, dtype=torch.long))
                                total_clusters = default_clusters
                                total_cluster_count = sum(len(clusters) for clusters in total_clusters)
                                # print(f"  Generated {total_cluster_count} default grid clusters")
                    except Exception as e:
                        # print(f"  ERROR in fallback threshold logic: {e}")
                        # import traceback
                        # traceback.print_exc()
                        pass
            ...
            patches, offsets = [], []
            # Determine target patch size (for default grid clusters, use cluster_w x cluster_h)
            # For adaptive clusters, we need to resize/pad to a consistent size
            target_h, target_w = None, None
            if mask_max == 0.0 and len(total_clusters) > 0:
                # Default grid clusters - all should be same size
                cluster_w = make_divisible(nx / self.ratio, 4)
                cluster_h = make_divisible(ny / self.ratio, 4)
                target_h, target_w = cluster_h, cluster_w
            else:
                # Adaptive clusters - find max size or use a standard size
                # Use the same size as default grid for consistency
                target_w = make_divisible(nx / self.ratio, 4)
                target_h = make_divisible(ny / self.ratio, 4)
            
            for bi, clusters in enumerate(total_clusters):
                if isinstance(clusters, torch.Tensor):
                    if len(clusters) > 0:
                        # clusters is tensor of shape [N, 4] with [x1, y1, x2, y2]
                        clusters_np = clusters.cpu().numpy()
                        for cluster in clusters_np:
                            x1, y1, x2, y2 = int(cluster[0]), int(cluster[1]), int(cluster[2]), int(cluster[3])
                            # Ensure valid bounds
                            x1, y1 = max(0, x1), max(0, y1)
                            x2, y2 = min(nx, x2), min(ny, y2)
                            if x2 > x1 and y2 > y1:  # Valid patch
                                patch = x[bi, :, y1:y2, x1:x2]
                                # Resize/pad patch to target size for consistent stacking
                                if target_h is not None and target_w is not None:
                                    patch_h, patch_w = patch.shape[1], patch.shape[2]
                                    if patch_h != target_h or patch_w != target_w:
                                        patch = F.interpolate(patch.unsqueeze(0), size=(target_h, target_w), mode='bilinear', align_corners=False).squeeze(0)
                                patches.append(patch)
                                offsets.append(torch.tensor([bi, x1, y1, x2, y2], device=device))
                    # else: empty tensor, skip
                elif isinstance(clusters, list):
                    # clusters is list of lists/tuples
                    for cluster in clusters:
                        if isinstance(cluster, (list, tuple)) and len(cluster) >= 4:
                            x1, y1, x2, y2 = int(cluster[0]), int(cluster[1]), int(cluster[2]), int(cluster[3])
                        elif isinstance(cluster, torch.Tensor):
                            cluster_list = cluster.tolist() if cluster.numel() >= 4 else [0, 0, nx, ny]
                            x1, y1, x2, y2 = int(cluster_list[0]), int(cluster_list[1]), int(cluster_list[2]), int(cluster_list[3])
                        else:
                            continue
                        # Ensure valid bounds
                        x1, y1 = max(0, x1), max(0, y1)
                    ...
                    try:
                        for cluster in clusters:
                            if hasattr(cluster, '__len__') and len(cluster) >= 4:
                                x1, y1, x2, y2 = int(cluster[0]), int(cluster[1]), int(cluster[2]), int(cluster[3])
                                x1, y1 = max(0, x1), max(0, y1)
                                x2, y2 = min(nx, x2), min(ny, y2)
                                if x2 > x1 and y2 > y1:
    ...
    def uni_slicer(self, feat, mask_pred, ratio=8, threshold=0.3, device='cuda'):
        ...
        if not hasattr(self, 'grid_off') or len(self.grid_off) != bs * ratio * ratio or self.grid_off.device != device:
            ...
            gy, gx = torch.meshgrid(xrange, xrange, indexing='ij')
