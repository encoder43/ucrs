...
try:
    ...
    # Fallback for when mmdet is not installed
    class MockRegistry:
        def register_module(self, force=False):
            def decorator(cls):
                return cls
            return decorator
    BACKBONES = MockRegistry()
...
try:
    ...
    # Fallback base class when GPViT is not available
    class GPViT(nn.Module):
        def __init__(self, *args, **kwargs):
            super().__init__()
            # Minimal attributes needed for the adapter to work
            self.layers = nn.ModuleList()
            self.embed_dims = 384  # Default dimension
            self.drop_path_rate = 0.1
            # Dummy patch_embed that returns a callable
            self.patch_embed = self._dummy_patch_embed
            self.pos_embed = None
            self.patch_resolution = (14, 14)  # Default patch resolution
            self.interpolate_mode = 'bicubic'
            self.drop_after_pos = nn.Identity()
        
        def _dummy_patch_embed(self, x):
            # Returns (features, patch_resolution) as expected
            B, C, H, W = x.shape
            # Return dummy feature tensor and resolution
            dummy_features = torch.zeros(B, (H // 8) * (W // 8), self.embed_dims, device=x.device)
            return dummy_features, (H // 8, W // 8)
            
        def _add_level_embed(self, *args):
            # Dummy method
...
class GPViTAdapterSingleStageESOD(GPViTAdapter):
    ...
    def feat_slice(self, featmaps: List[torch.Tensor], clusters: torch.Tensor, scales: List[int]):
        ...
        if getattr(self, 'fs_grids', None) is None or self.fs_grids[0].size(1) != w*h:
            self.fs_grids = []
            for s in scales:
                gy, gx = torch.meshgrid(torch.arange(h//s), torch.arange(w//s), indexing='ij')
                gxy = torch.stack((gy.reshape(-1), gx.reshape(-1)), dim=0)  # shape(2, w*h)
