...
try:
    with warnings.catch_warnings():
        ...
        warnings.filterwarnings('ignore', message='.*mmcv.*')
        from models.gpvit import *
except ImportError:
    # Optional dependency - only warn if actually trying to use GPViT
    SpatialPriorModule = GPViTAdapterSingleStageESOD = None
from models.spconv import SPYOLOv5Head, SPYOLOv6Head
from models.experimental import *
try:
    from models.uncertainty_segmenter import UncertaintySegmenter
except ImportError:
    UncertaintySegmenter = None
    warnings.warn('UncertaintySegmenter not available. Install uncertainty modules for probabilistic slicing.')
...
class Detect(nn.Module):
    ...
    def get_indices(self, offsets, mask, thresh=0.3):
        ...
        if not hasattr(self, 'sparse_gird') or self.sparse_gird is None or self.sparse_gird[0].shape != (1,patch_h,patch_w):
            yv, xv = torch.meshgrid([torch.arange(patch_h), torch.arange(patch_w)], indexing='ij')
...
class Model(nn.Module):
    ...
    def forward_once(self, x, profile=False, hm_only=False):
        ...
        for mi, m in enumerate(self.model):
            ...
            # ESOD: HeatMapParser receives heatmaps (masks) for slicing
            # During training: Use GT masks if provided and valid
            # During inference: Use predicted masks from Segmenter
            if isinstance(m, HeatMapParser):
                # masks can be GT masks (from input) or pred_masks (from Segmenter)
                # Priority: Use GT masks if they exist and have non-zero values
                # Otherwise, fall back to predicted masks
                heatmaps_to_use = None
                if masks is not None:
                    # Check if GT masks are valid (not all zeros)
                    ...
                    if mask_check is not None and mask_check.numel() > 0:
                        # Check if masks have non-zero values (valid masks)
                        if mask_check.max() > 0.001:  # Has meaningful values
                            heatmaps_to_use = masks
                
                # Fall back to predicted masks if GT masks are invalid/all zeros
                if heatmaps_to_use is None and pred_masks is not None:
                    heatmaps_to_use = pred_masks
                
                if heatmaps_to_use is not None:
                    # Ensure heatmaps is a list format expected by HeatMapParser
                    if isinstance(heatmaps_to_use, list):
                        x = (x[0] if isinstance(x, (list, tuple)) else x, heatmaps_to_use)
                    else:
                        # Convert tensor to list format
                        x = (x[0] if isinstance(x, (list, tuple)) else x, [heatmaps_to_use])
                else:
                    # No valid masks available - use dummy mask (will trigger default grid clusters)
                    if isinstance(x, (list, tuple)):
                        x = (x[0], [torch.zeros_like(x[0][:, :1])])  # Dummy mask
                    else:
                        x = (x, [torch.zeros_like(x[:, :1])])  # Dummy mask
            elif type(m) in [Detect, Center] and offsets is not None:
                x = (x, offsets)
                # Ensure masks is in the format Detect expects: list with one element, not a tensor
                masks_to_pass = None
                if masks is not None:
                    ...
                    if isinstance(pred_masks, list) and len(pred_masks) > 0:
                        masks_to_pass = pred_masks
                    elif isinstance(pred_masks, torch.Tensor):
                        masks_to_pass = [pred_masks]
                    else:
                        masks_to_pass = None
                # Only pass masks if we have valid masks (Detect expects list with one element)
                if masks_to_pass is not None:
                    x = (*x, masks_to_pass)
                # If no masks, don't pass masks argument (Detect will work without it)
            ...
            if isinstance(m, Segmenter):
                # Segmenter produces predicted masks (objectness heatmaps)
                pred_masks = x
                
                # ESOD: During training, we have GT masks for loss computation
                # During inference, we use predicted masks for HeatMapParser
                # Store predicted masks for downstream use (HeatMapParser, Detect layer)
                
                # Debug: Check predicted mask values (inference mode, first call only)
                # if not self.training and not hasattr(self, '_debug_segmenter_printed'):
                #     if isinstance(pred_masks, list) and len(pred_masks) > 0:
                #         pred_mask_tensor = pred_masks[0]
                #         if isinstance(pred_mask_tensor, torch.Tensor):
                #             mask_max = pred_mask_tensor.max().item()
                #             mask_min = pred_mask_tensor.min().item()
                #             mask_mean = pred_mask_tensor.mean().item()
                #             print(f"\nDEBUG Segmenter: pred_masks stats: min={mask_min:.4f}, max={mask_max:.4f}, mean={mask_mean:.4f}")
                #             sigmoid_max = pred_mask_tensor.sigmoid().max().item()
                #             print(f"  After sigmoid: max={sigmoid_max:.4f}")
                #             print(f"  Shape: {pred_mask_tensor.shape}")
                #             # Check if predictions are all similar (potential training issue)
                #             if mask_max - mask_min < 0.001:
                #                 print(f"  WARNING: Mask values are nearly constant - Segmenter may not be trained!")
                #             # Check if sigmoid values are reasonable (should be > 0.1 for meaningful activation)
                #             if sigmoid_max < 0.1:
                #                 print(f"  WARNING: Sigmoid max ({sigmoid_max:.4f}) is very low - Segmenter may need better training!")
                #     self._debug_segmenter_printed = True
                
                if hm_only:
                    return (None, None), pred_masks
                
                # ESOD mask handling:
                # - If GT masks provided (training mode): Keep GT masks for loss, but also store pred_masks
                # - If no GT masks (inference mode): Use predicted masks for HeatMapParser
                if masks is None:
                    masks = pred_masks
                # Note: We keep both GT masks (for loss) and pred_masks (for downstream processing)
                # The loss computation in ComputeLoss will use GT masks, while HeatMapParser uses pred_masks
            elif UncertaintySegmenter is not None and isinstance(m, UncertaintySegmenter):
                # UncertaintySegmenter returns (mean_mask, uncertainty_mask)
                if isinstance(x, tuple) and len(x) == 2:
                    pred_masks_mean, pred_masks_uncertainty = x
                    # Store for loss computation and HeatMapParser access
                    self.pred_mask_mean = pred_masks_mean
                    self.pred_mask_uncertainty = pred_masks_uncertainty
                    pred_masks = pred_masks_mean  # Use mean for downstream processing (single tensor, not tuple)
                else:
                    pred_masks = x
                    # If single output, assume it's mean (backward compatibility)
                    self.pred_mask_mean = pred_masks
                    self.pred_mask_uncertainty = None
                if hm_only:
                    return (None, None), pred_masks
                if masks is None:
                    masks = pred_masks
            elif isinstance(m, HeatMapParser):
                # Pass uncertainty mask if available
                if UncertaintySegmenter is not None and hasattr(self, 'pred_mask_uncertainty'):
                    # Modify heatmaps tuple to include uncertainty info
                    if isinstance(x, tuple) and len(x) == 2:
                        feat, offsets = x
                    else:
                        offsets = x
                    # HeatMapParser will check for uncertainty internally
                if isinstance(x, torch.Tensor):
                    offsets = x
                    if offsets.size(0) == 0:
                        return (None, None), pred_masks
                elif isinstance(x, tuple) and isinstance(x[1], torch.Tensor):
                    x, offsets = x
                    if len(x) == 0:
                        return (None, None), pred_masks
                else:
        ...
        # Ensure pred_masks is a list (not tuple) for loss computation compatibility
        # Loss function expects p_seg to be a list with one element
        if pred_masks is not None:
            if isinstance(pred_masks, tuple):
                # If it's a tuple (mean, uncertainty), use only mean for loss
                pred_masks = pred_masks[0] if len(pred_masks) > 0 else None
            if pred_masks is not None and not isinstance(pred_masks, list):
                # Wrap single tensor in list for loss function
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        kwargs = {}  # Initialize kwargs for all modules
        ...
        elif UncertaintySegmenter is not None and m is UncertaintySegmenter:
            args.append([ch[x] for x in f])
            # UncertaintySegmenter args: [nc, ch], optional kwargs
            # Store kwargs separately to pass as keyword arguments
            if len(args) == 2:
                # Add default kwargs if not provided
                kwargs = {'method': 'evidential'}
            elif len(args) > 2 and isinstance(args[-1], dict):
                # Extract kwargs from args
                kwargs = args.pop()
            # else: kwargs already initialized to {}
        elif m is Center:
            args.append([ch[x] for x in f])
        ...
        # Instantiate module with args and kwargs
        if UncertaintySegmenter is not None and m is UncertaintySegmenter and kwargs:
